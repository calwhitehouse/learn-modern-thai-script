...
// --- thai-letter-phonetics.ts (letter reference table) ---
const phoneticsPath = path.join(root, "src", "lib", "thai-letter-phonetics.ts");
const phoneticEntries = letterRows
  .map(({ row }) => {
    const letter = row["Thai Letter"];
    const phonetic = row.Phonetic.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    return `  "${letter}": "${phonetic}",`;
  })
  .join("\n");
const phoneticsTs = `/** Romanized phonetic names for the letter reference table. Generated by npm run generate:consonants. */
...
`;
fs.writeFileSync(phoneticsPath, phoneticsTs);
...
console.log("  src/lib/thai-letter-phonetics.ts");
